...
// DataentryLogic - Create a single entry (check) yrtyrty
  ...
  try {
    ...
    for (let i = 0; i < validatedEntries.length; i += batchSize) {
      ...
      try {
        ...
        if (batchError.name === "BulkWriteError") {
          ...
          if (batchError.writeErrors) {
            batchError.writeErrors.forEach((err) => {
              errors.push(
                `Upload problem in batch ${i / batchSize + 1}: ${err.errmsg || "Some entries could not be saved. Please check your data."}`
              );
            });
          }
        } else {
          errors.push(
            `Upload problem in batch ${i / batchSize + 1}: Some entries could not be saved. Please check your data and try again.`
          );
        }
      }
    }
